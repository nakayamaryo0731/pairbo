...
| 監査ログ           | 重要操作（グループ作成、精算、管理者オーバーライド等）のログ記録 |
...
## リリース前セキュリティレビュー結果（2026-02-15）

### レビュー対象と結果

| カテゴリ                      | 状態    | 備考                                                  |
| ----------------------------- | ------- | ----------------------------------------------------- |
| 認証 (Clerk)                  | ✅ 良好 | 全エンドポイントで `authMutation`/`authQuery` 使用    |
| 認可 (グループメンバーシップ) | ✅ 良好 | `requireGroupMember`/`requireGroupOwner` 一貫使用     |
| 入力バリデーション            | ✅ 良好 | ドメインルールで厳密にバリデーション                  |
| データ分離                    | ✅ 良好 | 全クエリが groupId でフィルタ、クロスグループ漏洩なし |
| 監査ログ                      | ✅ 良好 | 構造化JSONログ、主要操作を記録                        |
| Webhook                       | ✅ 良好 | Stripe署名検証済み                                    |
| シークレット管理              | ✅ 良好 | `.env*` gitignore、ハードコードなし                   |

### 個別判断事項

#### 招待トークン（`invitations.getByToken`）の安全性

- 招待トークンはUUID v4（122bitエントロピー）を使用しており、ブルートフォースによる推測は現実的に不可能
- Convex の query は読み取り専用であり、レート制限はConvex Cloudインフラ側で適用される
- 招待受諾UIで「〇〇さんが△△グループに招待しています」と表示するため、グループ名・招待者名の返却は必要
- **結論**: 現状の実装で問題なし

#### `subscriptions.getMySubscription` が公開クエリであることの安全性

- 未認証時は `{ plan: "free", isAdmin: false }` の固定値を返すのみ
- 認証済みの場合でも自分のサブスク情報のみ返却
- ランディングページ等で未ログイン時にも呼ばれるため `query`（`authQuery` ではなく）にしている
- **結論**: 機密情報の露出なし、現状の実装で安全

#### 管理者プランオーバーライドの監査ログ

- `subscriptions.setAdminPlanOverride` に `ctx.logger.audit()` を追加済み
- 管理者がプランを変更した際の記録が監査ログに残る
...
| 2026-02-15 | リリース前セキュリティレビュー実施、管理者オーバーライド監査ログ追加 | Claude |
